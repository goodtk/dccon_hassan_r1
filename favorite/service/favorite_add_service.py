...
def add_favorite(ctx, shortcut_name, package_name, dccon_name):
    ...
    elif registed_count > 0 and is_exactly_same_exist(author_id, shortcut_name):     # 해당 단축어가 이미 추가되어있는지 체크
        log(ctx, f'delete_favorite "{shortcut_name}" already exists.')
        return shortcut_name + ' 단축어으로 등록된 즐겨찾기가 이미 존재합니다.'
...
# 추가 시 중복 체크
def is_exactly_same_exist(author_id, shortcut_name):
    ...
    for line in lines:
        ...
        if splited[0] == shortcut_name:
            system_log(f'add_favorite {shortcut_name} of {author_id} is already exists.')
            exist = True
            break
    file.close()
