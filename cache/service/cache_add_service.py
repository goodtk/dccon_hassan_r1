...
# 캐시 추가
def add_cache(package_name, idx, file_name, bytes, cache_directory, cache_max):
    if cache_max == 0:
        return

    if not os.path.isdir(cache_directory):
        if not create_directory(cache_directory):                                        # 캐시 폴더가 존재하지 않으면 생성. 생성 실패시 오류메시지 출력.
            system_log('creating cache directory failed')

    # 캐시 개수 체크
    if _get_count_of_cache(cache_directory) >= cache_max:
        _remove_latest_cache(cache_directory)

    # 이미지 저장
    cache_path = os.path.join(cache_directory, f'{file_name}')
    with open(cache_path,'wb') as out: ## Open temporary file as bytes
        out.write(bytes.read())                         ## Read bytes into file

    # 캐시 인덱스에 append
    # 캐시 인덱스에 중복추가안하게 해라
    cache_idx_path = os.path.join(cache_directory, 'cacheIdx.txt')
    file = open(cache_idx_path, mode='at', encoding='utf-8')
    file.write(package_name + '\t' + idx + '\t' + file_name + '\n')
    file.close()

# 캐시 개수 조회
def _get_count_of_cache(cache_directory):
    cache_idx_path = os.path.join(cache_directory, 'cacheIdx.txt')
...
def _remove_latest_cache(cache_directory):
    cache_idx_path = os.path.join(cache_directory, 'cacheIdx.txt')
    file = open(cache_idx_path, mode='rt', encoding='utf-8')
    ...
    latest_cache_path = os.path.join(cache_directory, splited[2].rstrip('\n'))
